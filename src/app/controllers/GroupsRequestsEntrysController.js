    ...
    async create(req, res) {
        ...
        if (isBanned)
            return res.status(401).json({
                error: 'You are not allowed to request access. You have been banned from that group.',
            });
        ...
        if (group.is_private) {
            ...
            if (requestEntry)
                return res.status(401).json({
                    error: 'You have already applied to join this group. Wait for an administrator to accept your request',
                });
